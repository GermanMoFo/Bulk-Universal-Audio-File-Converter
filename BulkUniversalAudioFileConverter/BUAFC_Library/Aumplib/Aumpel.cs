...
namespace Arbingersys.Audio.Aumplib 
{
	...
	public class Aumpel
	{
		...
		Reporter(int totalBytes, int processedBytes, Aumpel aumpelObj, ref double progress);		
		...
		Convert(string inputFile, int convertFrom,
				string outputFile, int convertTo, Reporter updateStatus, ref double progress)
		{
			...
			if ((soundFormat)convertTo == soundFormat.MP3)
			{
				...
				((index = br.Read(wavBuffer, 0, mp3Writer.sampleSize)) > 0) 
				{
					...
					updateStatus(soundFileSize, processedBytes, this, ref progress);
			...
			else
			{
				...
				while ( (readCount = libSndFile.Read()) > 0) 
				{
					...
					updateStatus((int)totalFrames, (int)readIndex, this, ref progress);			
